...
#define TCP_MAX_ENTRY_TIME	250
#define MAX_TCP_PORT_OPEN_TIME 7500                                            // 30sekunden
...
struct tcp_table
{
   ...
   volatile unsigned char                  txretry;
...
void                                    stack_set_ip_settings(uint8_t * myIpPtr, uint8_t * myNetMask,
                                                              uint8_t * myRouterIp);
...
#define TCP_DATA_START_FIX    0x36
...
#define  PSH_FLAG				0x08                                                // Datenübergabe an Anwendung
                                                                               // beschleunigen (push)
#define  ACK_FLAG				0x10                                                // Datenübertragung bestätigen
                                                                               // (acknowledge)
...
struct Ethernet_Header
{
   ...
   unsigned int                            EnetPacketType;                     // Nutzlast 0x0800=IP Datagramm;0x0806 = 
                                                                               // ARP
};
...
struct ARP_Header
{
   unsigned int                            ARP_HWType;                         // Hardware Typ enthält den Code für
                                                                               // Ethernet oder andere Link Layer
   unsigned int                            ARP_PRType;                         // Protokoll Typ enthält den Code für IP 
                                                                               // o. anderes Übertragungsprotokoll
   unsigned char                           ARP_HWLen;                          // Länge der Hardwareadresse enthält 6
                                                                               // für 6 Byte lange MAC Adressen
   unsigned char                           ARP_PRLen;                          // Länge der Protocolladresse enthält 4
                                                                               // für 4 Byte lange IP Adressen
   unsigned int                            ARP_Op;                             // Enthält Code der signalisiert ob es
                                                                               // sich um eine Anfrage o. Antwort
                                                                               // handelt
   unsigned char                           ARP_SHAddr[6];                      // Enthält die MAC Adresse des
                                                                               // Anfragenden 
   unsigned long                           ARP_SIPAddr;                        // Enthält die IP Adresse des Absenders
   unsigned char                           ARP_THAddr[6];                      // MAC Adresse Ziel, ist in diesem Fall
                                                                               // 6 * 00,da die Adresse erst noch
                                                                               // herausgefunden werden soll (ARP
                                                                               // Request)
   unsigned long                           ARP_TIPAddr;                        // IP Adresse enthält die IP Adresse zu
                                                                               // der die Kommunikation aufgebaut
                                                                               // werden soll 
};
...
struct IP_Header
{
   ...
   unsigned int                            IP_Pktlen;                          // 16 Bit Komplette Läng des IP
                                                                               // Datagrams in Bytes
   ...
   unsigned char                           IP_ttl;                             // 8 Bit Time to Live die lebenszeit
                                                                               // eines Paket
   unsigned char                           IP_Proto;                           // Zeigt das höherschichtige Protokoll
                                                                               // an 
   // (TCP, UDP, ICMP)
...
struct ICMP_Header
{
   unsigned char                           ICMP_Type;                          // 8 bit typ identifiziert Aufgabe der
                                                                               // Nachricht 
   // 0 = Echo Reply 8 = Echo Request
   unsigned char                           ICMP_Code;                          // 8 Bit Code enthält Detailangaben zu
                                                                               // bestimmten Typen
   unsigned int                            ICMP_Cksum;                         // 16 Bit Prüfsumme enthält die CRC
                                                                               // Prüfsumme
...
struct TCP_Header
{
   unsigned int                            TCP_SrcPort;                        // der Quellport für die Verbindung
                                                                               // (Socket)
   unsigned int                            TCP_DestPort;                       // der Zielport für die Verbindung
                                                                               // (Socket)
   ...
   unsigned char                           TCP_HdrFlags;                       // 4Bit Länge des Headers in 32 Bit
                                                                               // schritten rest Flags
   unsigned int                            TCP_Window;                         // Fenstergröße max. 64K Byte
   unsigned int                            TCP_Chksum;                         // Enthält eine Prüfsumme über Header
                                                                               // und Datenbereich
...
struct UDP_Header
{
   unsigned int                            udp_SrcPort;                        // der Quellport für die Verbindung
                                                                               // (Socket)
   unsigned int                            udp_DestPort;                       // der Zielport für die Verbindung
                                                                               // (Socket)
   unsigned int                            udp_Hdrlen;
   unsigned int                            udp_Chksum;                         // Enthält eine Prüfsumme über Header
                                                                               // und Datenbereich
};
