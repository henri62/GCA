...
extern void                             make_udp_reply_from_request(uint8_t * buf, char *data, uint8_t datalen,
                                                                    uint16_t port);
extern uint16_t                         ip_arp_udp_tcp_get_source_port(uint8_t * buf);
...
#ifdef WWW_client
// ----- http get
extern void                             client_browse_url(prog_char * urlbuf, char *urlbuf_varpart, prog_char * hoststr,
...
extern void                             client_http_post(prog_char * urlbuf, prog_char * hoststr,
                                                         prog_char * additionalheaderline, char *postval,
                                                         void (*callback) (uint8_t, uint16_t, uint16_t));
// The callback is a reference to a function which must look like this:
// void browserresult_callback(uint8_t statuscode,uint16_t datapos,uint16_t len)
// statuscode=0 means a good webpage was received, with http code 200 OK
...
extern uint8_t                          send_udp(uint8_t * buf, char *data, uint8_t datalen, uint16_t sport,
