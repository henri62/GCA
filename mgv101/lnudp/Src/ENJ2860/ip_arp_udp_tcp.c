...
static uint8_t                          tcp_fd = 0;                            // a file descriptor, will be encoded
...
static uint8_t                          gwmacaddrmulti[6] = { 0x01, 0x00, 0x5e, 0x00, 0x00, 0x01 };
static uint8_t                          waitgwmac = 0;                         // 0=wait, 1=first req no anser, 2=have

                                                                               // gwmac, 4=refeshing but have gw mac,
                                                                               // 8=accept an arp reply
...
void make_eth_multicast(uint8_t * buf)
{
   uint8_t                                 i = 0;

   // 
   // copy the destination mac from the source and fill my mac into src
   buf[ETH_DST_MAC] = 0x01;
   buf[ETH_DST_MAC + 1] = 0x00;
   buf[ETH_DST_MAC + 2] = 0x5e;
   buf[ETH_DST_MAC + 3] = 0x00;
   buf[ETH_DST_MAC + 4] = 0x00;
   buf[ETH_DST_MAC + 5] = 0x01;
...
void fill_ip_hdr_checksum_udp(uint8_t * buf)
{
   uint16_t                                ck;
...
void make_tcp_ack_from_any(uint8_t * buf, int16_t datlentoack, uint8_t addflags)
{
   ...
   buf[TCP_WIN_SIZE] = 0x4;                                                    // 1024=0x400, 1280=0x500 2048=0x800
   // 768=0x300
...
void client_icmp_request(uint8_t * buf, uint8_t * destip)
{
   ...
   buf[ICMP_IDENT_L_P + 2] = 1;                                                // seq number, low byte, we send only 1
   // ping at a time
   // copy the data:
   i = 0;
   while (i < 56)
...
uint8_t client_ntp_process_answer(uint8_t * buf, uint32_t * time, uint8_t dstport_l)
{
   ...
   *time =
      ((uint32_t) buf[0x52] << 24) | ((uint32_t) buf[0x53] << 16) | ((uint32_t) buf[0x54] << 8) | ((uint32_t)
                                                                                                   buf[0x55]);
   return (1);
}
#endif
...
void send_udp_prepare(uint8_t * buf, uint16_t sport, uint8_t * dip, uint16_t dport)
{
   ...
   while (i < 6)
   {
      buf[ETH_DST_MAC + i] = gwmacaddrmulti[i];
      buf[ETH_SRC_MAC + i] = macaddr[i];
      i++;
   }
   buf[ETH_TYPE_H_P] = ETHTYPE_IP_H_V;
...
void client_set_gwip(uint8_t * gwipaddr)
{
   ...
   waitgwmac = WGW_INITIAL_ARP;                                                // causes an arp request in the packet
   // loop
   while (i < 4)
   {
      gwip[i] = gwipaddr[i];
...
//
uint8_t
client_tcp_req(uint8_t(*result_callback)
               (uint8_t fd, uint8_t statuscode, uint16_t data_start_pos_in_buf, uint16_t len_of_data),
...
uint8_t www_client_internal_result_callback(uint8_t fd, uint8_t statuscode, uint16_t datapos, uint16_t len_of_data)
{
   ...
   if (statuscode == 0 && len_of_data > 12)
   {
      ...
      if (client_browser_callback)
      {
         ...
         {
            (*client_browser_callback) (0, ((uint16_t) TCP_SRC_PORT_H_P + (bufptr[TCP_HEADER_LEN_P] >> 4) * 4),
                                        len_of_data);
         }
         else
         {
            (*client_browser_callback) (1, ((uint16_t) TCP_SRC_PORT_H_P + (bufptr[TCP_HEADER_LEN_P] >> 4) * 4),
                                        len_of_data);
         }
      }
   }
...
//
void client_browse_url(prog_char * urlbuf, char *urlbuf_varpart, prog_char * hoststr,
...
void client_http_post(prog_char * urlbuf, prog_char * hoststr, prog_char * additionalheaderline, char *postval,
                      void (*callback) (uint8_t, uint16_t, uint16_t))
{
   client_urlbuf = urlbuf;
   client_hoststr = hoststr;
...
/*
 * Get the port number of the source from the received packet. 
 */
uint16_t ip_arp_udp_tcp_get_source_port(uint8_t * buf)
{
   uint16_t                                Port;

   Port = ((uint16_t) buf[UDP_DST_PORT_H_P] << 8) + buf[UDP_DST_PORT_L_P];
   return (Port);
}
