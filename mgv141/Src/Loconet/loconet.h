...
#define GTRK_PROG_BUSY    0x08                                                 /* 1 = programming track in this master
                                                                                * is Busy */
#define GTRK_MLOK1        0x04                                                 /* 0 = Master is DT200, 1=Master
                                                                                * implements LocoNet 1.1 */
#define GTRK_IDLE         0x02                                                 /* 0 = Track paused, B'cast EMERG STOP,
                                                                                * 1 = Power On */
#define GTRK_POWER        0x01

#define FC_SLOT           0x7b                                                 /* Fast clock is in this slot */
#define PRG_SLOT          0x7c                                                 /* This slot communicates with the
                                                                                * programming track */

/* values and macros to decode programming messages */
#define PCMD_RW           0x40                                                 /* 1 = write, 0 = read */
#define PCMD_BYTE_MODE    0x20                                                 /* 1 = byte operation, 0 = bit operation 
                                                                                * (if possible) */
...
#define PSTAT_READ_FAIL     0x04                                               /* Failed to detect Read Compare
                                                                                * Acknowledge from decoder */
...
{
   byte                                    command;
   byte                                    sw1;                                /* ls seven bits of switch address
                                                                                * (D6-D0) */
   byte                                    sw2;                                /* ms four bits of switch address
                                                                                * (D3-D0) */
   /* and control bits */
   byte                                    chksum;                             /* exclusive-or checksum for the message 
                                                                                */
...
{
   byte                                    command;
   byte                                    opcode;                             /* op-code of message getting the
                                                                                * response (msb=0) */
...
typedef struct rwslotdata_t
{
   ...
   byte                                    ss2;                                /* slot status 2 (tells how to use
                                                                                * ID1/ID2 & ADV Consist */
...
typedef struct fastclock_t
{
   ...
   byte                                    mesg_size;                          /* ummmmm, size of the message in bytes? 
                                                                                */
   byte                                    slot;                               /* slot number for this request */
   byte                                    clk_rate;                           /* 0 = Freeze clock, 1 = normal, 10 =
                                                                                * 10:1 etc. Max is 0x7f */
   byte                                    frac_minsl;                         /* fractional minutes. not for external
                                                                                * use.  */
   byte                                    frac_minsh;
   ...
   /* " " 0; ignore this reply */
   byte                                    id1;                                /* id1/id2 is device id of last device
                                                                                * to set the clock */
...
typedef struct progtask_t
{
   ...
   byte                                    mesg_size;                          /* ummmmm, size of the message in bytes? 
                                                                                */
   byte                                    slot;                               /* slot number for this request - slot
                                                                                * 124 is programmer */
   byte                                    pcmd;                               /* programmer command */
   byte                                    pstat;                              /* programmer status error flags in
                                                                                * reply message */
   byte                                    hopsa;                              /* Ops mode - 7 high address bits of
                                                                                * loco to program */
   byte                                    lopsa;                              /* Ops mode - 7 low address bits of loco 
                                                                                * to program */
   byte                                    trk;                                /* track status. Note: bit 3 shows if
                                                                                * prog track is busy */
   byte                                    cvh;                                /* hi 3 bits of CV# and msb of data7 */
   byte                                    cvl;                                /* lo 7 bits of CV# */
   byte                                    data7;                              /* 7 bits of data to program, msb is in
                                                                                * cvh above */
   byte                                    pad2;
   byte                                    pad3;
   byte                                    chksum;                             /* exclusive-or checksum for the message 
...
typedef struct sv_t
{
   ...
   byte                                    dst_lo;                             /* low 8 bits of 16 bit destination
                                                                                * address */
   byte                                    dst_hi;                             /* high 8 bits of 16 bit destination
                                                                                * address */
...
typedef struct
{
   byte                                    command;                            // OPC_PEER_XFER for replies,
   // OPC_IMM_PACKET for commands
