...
unsigned near char sm_handle;
unsigned near int cv;
unsigned near char cv_register;
unsigned near char cv_page;
unsigned near char mode;
unsigned near char cv_data;

unsigned near char prog_state;
unsigned near char state_count;
unsigned near char cv_cmd, val;
...
void cmd_cv(void) {
    ...
    cv = (((unsigned int)rx_ptr->d2)<<8 | rx_ptr->d3) - 1;
    cv_data = rx_ptr->d5;
    if ((rx_ptr->d0 == OPC_WCVO) || (rx_ptr->d0 == OPC_WCVB)) {
