...
BYTE MACGet()
{
	SPISelectEthernet();
	SSPBUF = RBM;
	while(!PIR1bits.SSPIF);		// Wait until opcode/address is transmitted.
	PIR1bits.SSPIF = 0;
	SSPBUF = 0;				// Send a dummy byte to receive the register 
							//   contents.
	while(!PIR1bits.SSPIF);		// Wait until register is received.
	PIR1bits.SSPIF = 0;
	SPIUnselectEthernet();
...
WORD MACGetArray(BYTE *val, WORD len)
{
	...
	while(i<len)
	{
		...
		i++;
		val++;
		while(!PIR1bits.SSPIF);	// Wait until byte is received.
		PIR1bits.SSPIF = 0;
		*val = SSPBUF;
	};
...
void MACPut(BYTE val)
{
	...
	while(!PIR1bits.SSPIF);		// Wait until opcode/constant is transmitted.
	PIR1bits.SSPIF = 0;
	SSPBUF = val;			// Send the byte to be writen.
	while(!PIR1bits.SSPIF);		// Wait until byte is transmitted.
	PIR1bits.SSPIF = 0;
	SPIUnselectEthernet();
}//end MACPut
...
static void SendSystemReset(void)
{
	SPISelectEthernet();
	SSPBUF = SR;
	while(!PIR1bits.SSPIF);		// Wait until the command is transmitted.
	PIR1bits.SSPIF = 0;
	SPIUnselectEthernet();
}//end SendSystemReset
...
static REG ReadETHReg(BYTE Address)
{
	...
	while(!PIR1bits.SSPIF);		// Wait until the opcode/address is transmitted
	PIR1bits.SSPIF = 0;
	SSPBUF = 0;				// Send a dummy byte to receive the register 
							//   contents
	while(!PIR1bits.SSPIF);		// Wait until the register is received
	PIR1bits.SSPIF = 0;
	SPIUnselectEthernet();
...
static REG ReadMACReg(BYTE Address)
{
	...
	while(!PIR1bits.SSPIF);		// Wait for the dummy byte to be transmitted
	PIR1bits.SSPIF = 0;
...
static void WriteReg(BYTE Address, BYTE Data)
{
	...
	while(!PIR1bits.SSPIF);		// Wait until register is written.
	PIR1bits.SSPIF = 0;
	SPIUnselectEthernet();
}//end WriteReg
