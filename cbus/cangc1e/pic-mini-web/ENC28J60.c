...
void MACInit(void)
{
	...
	SSP1CON1 = 0x20;			// SSPEN bit is set, SPI in master mode, FOSC/4, 
...
BYTE MACGet()
{
	SPISelectEthernet();
	SSP1BUF = RBM;
	while(!SSP1IF);		// Wait until opcode/address is transmitted.
	SSP1IF = 0;
	SSP1BUF = 0;				// Send a dummy byte to receive the register 
							//   contents.
	while(!SSP1IF);		// Wait until register is received.
	SSP1IF = 0;
	SPIUnselectEthernet();
...
WORD MACGetArray(BYTE *val, WORD len)
{
	...
	SPISelectEthernet();
	SSP1BUF = RBM;			// Send the Read Buffer Memory opcode.
	i = 0;		
	val--;
	while(!SSP1IF);		// Wait until opcode/address is transmitted.
	SSP1IF = 0;
	...
	while(i<len)
	{
		SSP1BUF = 0;			// Send a dummy byte to receive a byte 
		i++;
		val++;
		while(!SSP1IF);	// Wait until byte is received.
		SSP1IF = 0;
		*val = SSP1BUF;
	};
...
{
	SPISelectEthernet();
	SSP1BUF = WBM;			// Send the opcode and constant.
	while(!SSP1IF);		// Wait until opcode/constant is transmitted.
	SSP1IF = 0;
	SSP1BUF = val;			// Send the byte to be writen.
	while(!SSP1IF);		// Wait until byte is transmitted.
	SSP1IF = 0;
	SPIUnselectEthernet();
}//end MACPut
...
void MACPutArray(BYTE *val, WORD len)
{
	...
	SPISelectEthernet();
	SSP1BUF = WBM;			// Send the Write Buffer Memory opcode
	while(!SSP1IF);		// Wait until opcode/constant is transmitted.
	SSP1IF = 0;
	...
	while(len)
	{
		SSP1BUF = *val;		// Start sending the byte
		val++;				// Increment after writing to SSP1BUF to increase speed
		len--;				// Decrement after writing to SSP1BUF to increase speed
		while(!SSP1IF);	// Wait until byte is transmitted
		SSP1IF = 0;
	};
...
static void SendSystemReset(void)
{
	SPISelectEthernet();
	SSP1BUF = SR;
	while(!SSP1IF);		// Wait until the command is transmitted.
	SSP1IF = 0;
	SPIUnselectEthernet();
}//end SendSystemReset
...
static REG ReadETHReg(BYTE Address)
{
	...
	while(!SSP1IF);		// Wait until the opcode/address is transmitted
	SSP1IF = 0;
	SSP1BUF = 0;				// Send a dummy byte to receive the register 
							//   contents
	while(!SSP1IF);		// Wait until the register is received
	SSP1IF = 0;
	SPIUnselectEthernet();
...
{
	SPISelectEthernet();
	SSP1BUF = RCR | Address;	// Send the Read Control Register opcode and 
							//   address.
	while(!SSP1IF);		// Wait until opcode/address is transmitted.
	SSP1IF = 0;
	SSP1BUF = 0;				// Send a dummy byte 
	while(!SSP1IF);		// Wait for the dummy byte to be transmitted
	SSP1IF = 0;
	SSP1BUF = 0;				// Send another dummy byte to receive the register 
							//   contents.
	while(!SSP1IF);		// Wait until register is received.
	SSP1IF = 0;
	SPIUnselectEthernet();
...
{
	SPISelectEthernet();
	SSP1BUF = WCR | Address;	// Send the opcode and address.
	while(!SSP1IF);		// Wait until opcode/address is transmitted.
	SSP1IF = 0;
	SSP1BUF = Data;			// Send the byte to be writen.
	while(!SSP1IF);		// Wait until register is written.
	SSP1IF = 0;
	SPIUnselectEthernet();
}//end WriteReg
...
{
	SPISelectEthernet();
	SSP1BUF = BFC | Address;	// Send the opcode and address.
	while(!SSP1IF);		// Wait until opcode/address is transmitted.
	SSP1IF = 0;
	SSP1BUF = Data;			// Send the byte to be writen.
	while(!SSP1IF);		// Wait until register is written.
...
{
	SPISelectEthernet();
	SSP1BUF = BFS | Address;	// Send the opcode and address.
	while(!SSP1IF);		// Wait until opcode/address is transmitted.
	SSP1IF = 0;
	SSP1BUF = Data;			// Send the byte to be writen.
	while(!SSP1IF);		// Wait until register is written.
